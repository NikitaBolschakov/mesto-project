export default class FormValidator {
  constructor(selectors, form) {
    ...
    this._inputList = Array.from(this._form.querySelectorAll(this._inputElement)); //массив с инпутами
  }
  ...
  _setEventListeners() {
    ...
    this._inputList.forEach((inputElement) => {    // Обойдём все элементы массива
  ...
  resetValidation() {
    ...
    this._inputList.forEach((inputElement) => {    //очищаем ошибки 
      this._hideInputError(inputElement);
  });
}
  ...
  enableValidation() {
    this._form.addEventListener("submit", (evt) => {  // Найдём форму с указанным классом в DOM,
      evt.preventDefault();
    });
